 ...
 *  Подсказка оказалась не совсем провильной. Пробывал вынестм в метод считование файлов,
 * но тогда я не могу организовать храненин таким образом, чтобы результот по отчетам
 * был атдельно за каждый меляц.
 * Если в мапе ключем делать название товара но это просто провал, так как ключи не могут
 * повторятся, они униакальны.
 *  А также проверка пустой ли текст почему то роботает не совсем коректно. Код говорит
 *  о пустом файле, но при этом продолжает работоть
 * Я был бы очень признателен, еслы ты Маким подсказал бы,
 * как можно оргонизовать систему хранения!!!
 *
 */
